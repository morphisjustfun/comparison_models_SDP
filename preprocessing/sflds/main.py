...
def run(max_tokens=2600, versions=None):
    if versions is None:
        versions = ['1.0', '1.2', '1.4', '1.6']
    function_instance_invocation_classes = [MethodInvocation, SuperMethodInvocation, ClassCreator]
    declaration_classes = [PackageDeclaration, InterfaceDeclaration, ClassDeclaration, ConstructorDeclaration,
                           MethodDeclaration, VariableDeclarator, VariableDeclaration, FormalParameter]
    control_flow_classes = [IfStatement, ForStatement, WhileStatement, DoStatement, AssertStatement, BreakStatement,
    ...
    other_classes = [BasicType, MemberReference, ReferenceType, SuperMemberReference, StatementExpression]
    ...
    def getDfsFilteredNodes(root):
        ...
        for node in dfsAstNode(root):
            try:
                if any(isinstance(node, classWanted) for classWanted in function_instance_invocation_classes):
                    if isinstance(node, ClassCreator):
                        dfsSequence.append(node.type.name)
                    else:
                        dfsSequence.append(node.member)
                elif any(isinstance(node, classWanted) for classWanted in declaration_classes):
                    if isinstance(node, LocalVariableDeclaration) or isinstance(node, VariableDeclaration):
                        dfsSequence.append(node.type.name)
                    else:
                        dfsSequence.append(node.name)

                elif any(isinstance(node, classWanted) for classWanted in control_flow_classes):
                ...
                elif any(isinstance(node, classWanted) for classWanted in other_classes):
    ...
    def getCammel(camel_version):
        ...
        trainData = pd.concat([trainData, pd.DataFrame(columns=[str(i) for i in range(max_tokens * 2)])])
        ...
        for i in range(len(dfs)):
            for j in range(len(dfs[i])):
                ...
                for k in range(len(dfsEncoded)):
                    dfs[i].at[j, str(k + max_tokens)] = dfsEncoded[k]

        df = pd.concat(dfs)
        df = df[[c for c in df if c not in ['bug']] + ['bug']]
        df.to_csv('dist/sflds/df.csv', index=False)
        train, test = train_test_split(df, test_size=0.2)
        train.to_csv('dist/sflds/df_train.csv', index=False)
        test.to_csv('dist/sflds/df_test.csv', index=False)
